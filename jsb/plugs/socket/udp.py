...
from jsb.contrib import rijndael
...
if cfg['udp'] and cfg['udpseed']: crypt = rijndael.rijndael(cfg['udpseed'])
...
def handle_udpenable(bot, event):
    cfg['udp'] = 1
    cfg.save()
    bot.plugs.reload("jsb.plugs.socket.udp")
    event.reply("the udp enable cfg is set, upd plugin reloaded")

cmnds.add("udp-enable", handle_udpenable, "OPER")
examples.add("udp-enable", "enable the udp plugin", "udp-enable")

## udp-disable plugin

def handle_udpdisable(bot, event):
    cfg['udp'] = 0
    cfg.save()
    bot.plugs.unload("jsb.plugs.socket.udp")
    event.reply("the udp disable cfg is set. udp plugin unloaded")

cmnds.add("udp-disable", handle_udpdisable, "OPER")
examples.add("udp-disable", "disable the udp plugin", "udp-disable")
